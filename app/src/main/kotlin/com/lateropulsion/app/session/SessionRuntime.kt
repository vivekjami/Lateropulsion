...
class SessionRuntime @Inject constructor(
    ...
    suspend fun resolveProfiles(): Pair<DeviceProfile, HeadsetProfile> {
        ...
        if (!dev.signResolved && s.fieldCalibratedSign != 0 && s.fieldCalibratedMountDeg != null) {
            dev = dev.copy(rollSign = s.fieldCalibratedSign, thetaMountDeg = s.fieldCalibratedMountDeg, notes = dev.notes + " [field-calibrated ${s.fieldCalibratedAt}]")
        }
