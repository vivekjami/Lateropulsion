...
/**
 * Three answers are enough to start: name, age, which way the patient pushes (ADR-020). Everything else has a
 * clinical default derived from those and lives under "More details".
 */
data class RegisterForm(
    val name: String = "", val age: String = "", val direction: Side = Side.LEFT,
    val sex: Sex = Sex.UNSPECIFIED, val mrn: String = "", val contact: String = "",
    val diagnosis: String = DEFAULT_DIAGNOSIS, val lesionSide: LesionSide? = null, val affectedSide: Side? = null,
    ...
    /** Pushers push toward the paretic side, which is opposite the lesion; the operator can still override. */
    val effectiveAffected: Side get() = affectedSide ?: direction
    val effectiveLesion: LesionSide get() = lesionSide ?: if (direction == Side.LEFT) LesionSide.RIGHT else LesionSide.LEFT

    companion object { const val DEFAULT_DIAGNOSIS = "Stroke with lateropulsion (pusher behaviour)" }
}
...
class RegisterPatientViewModel @Inject constructor(private val patients: PatientRepository, private val clock: Clock) : ViewModel() {
    ...
    fun create() = viewModelScope.launch {
        ...
        val patient = Patient(
            id = id, displayId = displayId, age = age, sex = f.sex, diagnosis = f.diagnosis.trim().ifBlank { RegisterForm.DEFAULT_DIAGNOSIS }, lesionSide = f.effectiveLesion,
            affectedSide = f.effectiveAffected, lateropulsionDirection = f.direction, onsetDate = onset, consentMedia = f.consentMedia, consentResearch = f.consentResearch,
...
fun RegisterPatientScreen(nav: NavHostController, vm: RegisterPatientViewModel = hiltViewModel()) {
    ...
    f.created?.let { id -> nav.navigate(Routes.profile(id)) { popUpTo(Routes.DASHBOARD) }; return }
    LpScreen(stringResource(R.string.registration), onBack = { nav.popBackStack() }) { mod ->
        Column(mod.verticalScroll(rememberScrollState()).padding(vertical = 8.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            StepHeader(1, 3, stringResource(R.string.step_register), stringResource(R.string.step_register_hint))
            ...
            Expander(stringResource(R.string.more_details), "${f.diagnosis} · lesion ${f.effectiveLesion.name.lowercase()} · paretic ${f.effectiveAffected.name.lowercase()}") {
                ...
                Selector(stringResource(R.string.lesion_side), LesionSide.entries, f.effectiveLesion, { it.name.lowercase() }, { v -> vm.update { it.copy(lesionSide = v) } })
                Selector(stringResource(R.string.affected_side), Side.entries, f.effectiveAffected, { it.name.lowercase() }, { v -> vm.update { it.copy(affectedSide = v) } })
                LpTextField(f.onset, { v -> vm.update { it.copy(onset = v) } }, stringResource(R.string.onset_date))
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    LpTextField(f.mrn, { v -> vm.update { it.copy(mrn = v) }; vm.checkDuplicates() }, stringResource(R.string.mrn), Modifier.weight(1f))
                    LpTextField(f.contact, { v -> vm.update { it.copy(contact = v) } }, stringResource(R.string.contact), Modifier.weight(1f))
                }
            ...
            Text(stringResource(R.string.id_auto_assigned), style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
            BigButton(stringResource(R.string.create_patient), { vm.create() }, Modifier.fillMaxWidth(), enabled = f.name.trim().length >= 2 && f.age.isNotBlank())
        }
    }
}
