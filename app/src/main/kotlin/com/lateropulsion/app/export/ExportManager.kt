...
class ExportManager @Inject constructor(@ApplicationContext private val ctx: Context, private val audit: AuditRepository, private val clock: Clock) {
    suspend fun share(fileName: String, mime: String, kind: String, targetType: String, targetId: String, deidentified: Boolean, clinician: ClinicianId?, write: (File) -> Unit): Result<Uri> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            LpLog.i(TAG, "export shared", "kind" to kind, "bytes" to f.length())
            uri
        }.onFailure { LpLog.e(TAG, "export failed", it, "kind" to kind) }
    }

    private companion object { const val TAG = "Export" }
}
